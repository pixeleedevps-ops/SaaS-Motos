...
revoke all on function public.empleado_actual_id() from public;
...
begin
  ...
  loop
    ...
    values (v_balance.variante_id, p_sede_destino_id, v_balance.stock, v_balance.stock_minimo)
...
revoke all on function public.trasladar_producto_entre_sedes(uuid, uuid, uuid) from public;
...
revoke all on function public.mover_producto_sede(uuid, uuid) from public;
...
revoke all on function public.crear_producto_inventario(text, text, text, uuid, uuid, uuid, numeric, numeric, text, integer, integer, boolean) from public;
